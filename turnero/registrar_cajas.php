        	...
        	<?php
        		...
        		if(isset($_POST['registrar'])){
					...
					if(verificar_datos($data)){
						...
						$error = "Error al seleccionar los Modulos";
						...
						if($noCajas == 0){	
							...
							if(mysqli_num_rows($buscar) < $noCajasPermitidas){
								...
								$error = "Error al registrar el Modulo";
							
								$registrar = consulta($con,$sql,$error);
							
								if($registrar == true){
							
									$mensaje = "<span class='correcto'>Modulo registrado</span>";
							
								}else{
							
									$mensaje = "<span class='mensaje'>error al registrar el Modulo</span>";
							
								}
							
							}else{

								$mensaje = "<span class='mensaje'>No se pueden registrar mas Modulos</span>";
							
							}
						
						}else{
						
							$mensaje = '<span class="mensaje">El Modulo ya fue registrado</span>';
			...
			?>
	        	...
	        	<form action="<?php $_SERVER['PHP_SELF']?>" method="post" name="formLogin" id="formLogin" class="form-login">
	                ...
	                <div class="contenedor-controles">
	                	...
	                	<label>Nombre:</label><input type="text" name="nombreCaja" id="nombreCaja" placeholder="Ingrese el nombre del Modulo">
